...
import static com.boke.constant.RabbitMQConstant.SUBSCRIBE_EXCHANGE;
import static com.boke.constant.RedisConstant.*;
import static com.boke.enums.ArticleStatusEnum.*;
import static com.boke.enums.StatusCodeEnum.ARTICLE_ACCESS_FAIL;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public PageResultDTO<ArchiveDTO> listArchives() {
        ...
        return new PageResultDTO<>(archiveDTOs, asyncCount.get().intValue());
    }
    ...
    public void saveOrUpdateArticle(ArticleVO articleVO) {
        ...
        if (article.getStatus().equals(1)) {
            ...
            //公开的文章推送到es
            ArticleSearchDTO artDto = BeanCopyUtil.copyObject(article, ArticleSearchDTO.class);
            elasticsearchUtil.addDocument(ARTICLE_INDEX, artDto.getId().toString(), artDto);

        }else{
            ArticleSearchDTO document = elasticsearchUtil.getDocument(ARTICLE_INDEX, article.getId().toString(), ArticleSearchDTO.class);
            if(document!=null){
                //如果文章由公开修改为私密，则删除es
                elasticsearchUtil.deleteDocument(ARTICLE_INDEX, document.getId().toString());
            }
        }
    }
    ...
    public void updateArticleDelete(DeleteVO deleteVO) {
        ...
        if(deleteVO.getIsDelete()==0){
            ...
            queryWrapper.in(Article::getId, deleteVO.getIds());
            List<Article> articleList = this.list(queryWrapper);
            List<ArticleSearchDTO> articleSearchDTOS = articleList.stream().map(item -> BeanCopyUtil.copyObject(item, ArticleSearchDTO.class)).toList();
            elasticsearchUtil.addBatchDocuments(ARTICLE_INDEX, articleSearchDTOS);
        }else{
            //批量删除
            elasticsearchUtil.deleteBatchDocument(ARTICLE_INDEX, deleteVO.getIds().stream().map(Object::toString).collect(Collectors.toList()));
        }
    }
    ...
    public void deleteArticles(List<Integer> articleIds) {
        ...
        elasticsearchUtil.deleteBatchDocument(ARTICLE_INDEX, articleIds.stream().map(Object::toString).collect(Collectors.toList()));
    }
